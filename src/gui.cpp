...
void snapImGuiWindowToEdge() {
	ImGuiIO& io = ImGui::GetIO();
	ImVec2 windowPos = ImGui::GetWindowPos();
	ImVec2 windowSize = ImGui::GetWindowSize();

	float snapRange = 20.0f;  // Pixels within snapping range
	ImVec2 targetPos = windowPos;

	// Check snapping for edges
	if (windowPos.x <= snapRange) targetPos.x = 0;  // Left edge
	if (windowPos.y <= snapRange) targetPos.y = 0;  // Top edge
	if (windowPos.x + windowSize.x >= io.DisplaySize.x - snapRange)
		targetPos.x = io.DisplaySize.x - windowSize.x;  // Right edge
	if (windowPos.y + windowSize.y >= io.DisplaySize.y - snapRange)
		targetPos.y = io.DisplaySize.y - windowSize.y;  // Bottom edge

	// Apply snapping
	if (targetPos.x != windowPos.x || targetPos.y != windowPos.y) {
		ImGui::SetWindowPos(targetPos);
	}
}

void loadModelButtonUI() {
	ImGui::Begin("Model Loader");
	snapImGuiWindowToEdge();
	if (ImGui::Button("Load Model")) {
		const char* filters[1] = { "*.obj" };  // Filter for OBJ files
		const char* filePath = tinyfd_openFileDialog("Choose a 3D Model", "", 1, filters, "OBJ Files", 0);

		if(filePath) {
			Model loadedModel = loadModel(filePath);
			if (!loadedModel.vertices.empty()) {
				std::cout << "Model loaded with " << loadedModel.vertices.size() << " vertices" << std::endl;

				Scene::objects.push_back(Scene::Object(
					3, // Type: Model
					{ 0.0f, 0.0f, 0.0f }, // Position
					{ 1.0f, 1.0f, 1.0f }, // Scale
					Scene::Material({ 0.8f, 0.8f, 0.8f }) // Material
				));

				// Assign the model to the last created object
				Scene::objects.back().model = loadedModel;
