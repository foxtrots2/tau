...
uniform bool isModelRenderingPass;
...
void main() {
	    if (isModelRenderingPass) { 
        // Direct rendering logic for 3D models
        fragColor = texture(u_modelTexture, fragUV);
    } else {
        // Existing ray tracing logic
        vec2 centeredUV = (fragUV * 2 - vec2(1)) * vec2(u_aspectRatio, 1.0);

        // Example ray tracing logic
        vec3 rayDir = (normalize(vec4(centeredUV, -1.0, 0.0)) * u_rotationMatrix).xyz;
        Ray cameraRay = Ray(u_cameraPosition, rayDir);

        fragColor = vec4(computeSceneColor(cameraRay, u_time), 1.0);
	}
	vec2 centeredUV = (fragUV * 2 - vec2(1)) * vec2(u_aspectRatio, 1.0);
